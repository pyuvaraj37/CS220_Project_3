...
#import sys
#sys.path.insert(0, '/extra/pyuva001/CS220_Project_3'
...
class CIFARReader(AbstractDataReader): 
    def __init__(self, args, load_from_snapshot=False):
        '''
            Args: 
                args: the argparser arguments
    ...
    def load_np_arrays(self, shuffle=True): 
        ...
        # writing feature names - pixel_<width>-<height>-<color>
        for i in range(32): 
            for j in range(32):
                for k in range(3):
                    for l in range(8):
                        self.feature_names.append('pixel_w{}-h{}-c{}-b{}'.format(i, j, k, l))
